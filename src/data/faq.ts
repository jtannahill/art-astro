...
export const FAQ: FaqEntry[] = [
  ...
  },
  {
    ...
    a: `${SCAN_POINTS} points across the globe are scanned every day. A composite visual interest score ranks them by pressure anomaly (${SCORE_WEIGHTS.pressure_anomaly}%), wind speed (${SCORE_WEIGHTS.wind}%), temperature deviation (${SCORE_WEIGHTS.temperature_deviation}%), precipitation (${SCORE_WEIGHTS.precipitation}%), and humidity (${SCORE_WEIGHTS.humidity}%). The top ${TOP_LOCATIONS_PER_DAY} &mdash; with at least ${MIN_LAT_LNG_SEPARATION_DEG}&deg; of geographic separation &mdash; become that day's subjects.`,
  },
  {
    q: "Can I buy a print?",
    a: `Yes. Every artwork page has a print shop &mdash; just click any artwork and look for the print option. Limited edition gicl&eacute;e prints on Hahnem&uuml;hle German Etching 310gsm, edition of ${EDITION_SIZE} per size, with Certificate of Authenticity. Ships worldwide via theprintspace. <a href="/archive/">Browse the archive</a> to find your piece.`,
  },
  {
  ...
  },
  {
    ...
    a: `A parallel system captures true-color imagery from the Copernicus Sentinel-2 satellite (${SATELLITE_ALTITUDE_KM} km altitude) and extracts the dominant color palettes. Deserts, glaciers, river deltas &mdash; the actual colors of Earth as seen from orbit. Browse them at <a href="/palettes/">Palettes</a>.`,
  },
  {
